...
try:
    import sys
    cmd = sys.argv[1]
    if not cmd in ["update", "clone", "clone_ff"]:
        del sys
        ""+1
except:
    print("no valid cmd specified | valid cmds clone, clone_ff, update")
    print("usage 4git.py clone git_repo_parent_url directory count (clones the whole repository of the holder)")
    print("usage 4git.py clone_ff git_repo_parent_url directory count (clones all repositories of the holder's social connections)")
...
if cmd == "update":
    ...
    for fua in follow_url_add:
        ...
        subprocess.Popen(cmd, cwd=src_dirs)
        del subprocess
        r=str(urllib_get(url+follow_url_add[fua]))
        if next_page_regx:
            next_page=next_page_regx.findall(r)
        follow=ppl_regx.findall(r)
        if follow != []:
            ppl_done=[]
            for f in range(len(follow)):
                if f % 2 == 0:
                    continue
                else:

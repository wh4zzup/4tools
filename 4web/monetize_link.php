...
class link_monetizer {
    private $ebay_campid;
    private $ebay_customid;
    private $amazon;

    public function __construct() {
        //eBay Data
        $ebay_campid="yourcampaignid";
        $ebay_customid="yourcustomid";
        //Amazon Data - please read the affiliate documentations - you need to create a sub array for each market
        $amazon = array('de' => array(  '_encoding' =>  'UTF8',     // needed value string
                        ...
                        'fr' => array(  '_encoding' =>  'UTF8',     // needed value string
                                        'tag'       =>  'tag_name', // needed value string | e.g. campaign tag name
                                        'linkCode'  =>  'ur2',      // mandatory value string | ur2 = textlink
                                        //'linkId'    =>  'link_id',
                                        'camp'      =>  0,          // needed value integer
    ...
    private function get_ebay_zone_data($country_code) {
    ...
    public function monetize_link($link){
        ...
        if(preg_match("/www.?(\w+.|)ebay./", $purl['host'])){
            ...
            list($prefix, $param) = explode('?', $link);
            parse_str($param, $ps);
            $ps['mkcid']=1;
            $zone_ids = $this->get_country_ebay($country);
            if($zone_ids){
                $ps['mkrid']=$zone_ids[0];
                $ps['siteid']=$zone_ids[1];
            }
            $ps['campid']=$this->$ebay_campid;
            $ps['customid']=$this->$ebay_customid;
            $ps['toolid']=10001;
            $ps['mkevt']=1;
